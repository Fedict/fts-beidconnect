#include "VirtualCard.hpp"
#include "log.hpp"
#include "util.h"
#include "hash.h"
#include "general.h"
#include "util.h"

constexpr char test[] = "test";
constexpr char id_file[] = "AQwwMDAwMDEwMTQ3NTkCEFNMR5BwZQAAtEUYbBKScTADCjMwLjA1LjIwMTcECjMwLjA1LjIwMjcFBkxldXZlbgYLOTgwODIxMDAwMjkHElNwZWNpbWVuIGVJRCAxOC03NQgKQWxpY2UgQmVydAkBQwoEQkVMRwsGTGV1dmVuDAsyMSBBVUcgMTk5OA0BVg4ADwExEAEwESBHaixAcvX7B2QuJW1e39ecamdfxpJi0yf5l5BRUXsIAQ==";
constexpr char id_sig_file[] = "";
constexpr char address_file[] = "ARFTcGVjaW1lbnN0cmFhdCAxMgIEMDAwMAMGVXRvcGlhAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA==";
constexpr char address_sig_file[] = "";
constexpr char photo_file[] = "";
constexpr char rrn_cert[] = "";
constexpr char auth_cert[] = "";
constexpr char nonrep_cert[] = "";
//eID SubCA
constexpr char ca_cert[] = "";
//eID root ca
constexpr char root_cert[] = "";

std::string VirtualCard::strType() const
{
    return "BEID";
};

int VirtualCard::type() const
{
    return CARD_TYPE_BEID;
    //return CARD_TYPE_PKCS15;
};

#define WHERE "VirtualCard::readCertificateChain"
void VirtualCard::readCertificateChain(std::vector<std::shared_ptr<const CardFile>>& subCerts, std::shared_ptr<const CardFile>& rootCert)
{
    subCerts.push_back(getFile(CardFiles::Cacert));
    rootCert = getFile(CardFiles::Rootcert);
    do_sleep(1000); //virtual read time
}
#undef WHERE

#define WHERE "VirtualCard::SelectKey()"
void VirtualCard::selectKey(CardKeys pintype, const std::vector<unsigned char>& cert)
{
#define eaZyID_PROVIDES_SIGNING_CERT    0

#if (eaZyID_PROVIDES_SIGNING_CERT)
    //compare first 500 bytes with certificate we want to use to make sure we are using the correct card
    //This should be changed to find out if we want to do an authentication or non-rep signing depending on input certificate
    if (memcmp(buf, cert, 500) != 0) {
        log_error("I: Virtual card does not contain the key for the requested certificate");
        ret = -1;
        goto cleanup;
    }
#endif
}

void VirtualCard::logon(int l_pin, char* pin)
{
    //test Pin conditions
    if ((l_pin > 0) && (l_pin < 4))
        throw BeidConnectException(BeidConnectException_Code::E_PIN_TOO_SHORT);
    if (l_pin > 12)
        throw BeidConnectException(BeidConnectException_Code::E_PIN_TOO_LONG);


    if ((l_pin == 0) && (reader->isPinPad())) {
        return;
    }
    else if (pin != 0) {

        if (strcmp(pin, "1234") == 0)
            return;
        else if (strcmp(pin, "1235") == 0)
            throw CardException(0x63C2);
        else if (strcmp(pin, "1236") == 0)
            throw CardException(0x63C1);
        else if (strcmp(pin, "1237") == 0)
            throw CardException(0x6983);
        else
            throw CardException(0, CardException_Code::PIN_Incorrect);
    }
    throw CardException(0, CardException_Code::PIN_Incorrect);
}

void VirtualCard::logoff()
{
}

long VirtualCard::sign(const std::vector<unsigned char>& in, int hashAlgo, unsigned char* out, size_t* l_out, int* sw)
{
    long ret = 0;
    char rand[256] = "the quick brown fox or something...";

    if ((in.size() == 0) || (in.size() != hash_length_for_algo(hashAlgo))) {
        log_error("hash input has wrong length for the specified digesting algo");
        CLEANUP(E_DIGEST_LEN);
    }

    memcpy(out, rand, 256);
    *l_out = (unsigned int)256;
    *sw = 0x9000;

    do_sleep(300); //virtual read time

cleanup:

    return (ret);
}

std::shared_ptr<const CardFile> VirtualCard::getFile(CardFiles fileType)
{
    do_sleep(200); //virtual read time

    switch (fileType)
    {
    case CardFiles::Id: return std::make_shared<CardFile>(id_file);
    case CardFiles::Id_sig: return std::make_shared<CardFile>(id_sig_file);
    case CardFiles::Address: return std::make_shared<CardFile>(address_file);
    case CardFiles::Address_sig: return std::make_shared<CardFile>(address_sig_file);
    case CardFiles::Photo: return std::make_shared<CardFile>(photo_file);
    case CardFiles::Rrncert: return std::make_shared<CardFile>(rrn_cert);
    case CardFiles::Authcert: return std::make_shared<CardFile>(auth_cert);
    case CardFiles::Signcert: return std::make_shared<CardFile>(nonrep_cert);
    case CardFiles::Cacert: return std::make_shared<CardFile>(ca_cert);
    case CardFiles::Rootcert: return std::make_shared<CardFile>(root_cert);
    default:
    {
        throw CardFileException("Unknow file requested");
    }
    }

    return nullptr;
}
