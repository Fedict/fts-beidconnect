...
const std::string oid2PrintableStr(unsigned char* oid, size_t l_oid)
{
	//int l_str = 128;
	int i;
	T_OID* p_entry = NULL;
	...
	for (n = 1; ; n++)
	{
		...
		for (i = 1;; i++)
		{
			...
			if (item.tag == ASN_SEQUENCE)
			{
				...
				strcpy(abrv, oid2PrintableStr(item.p_data, item.l_data).c_str());
				l_abrv = strlen(abrv);
